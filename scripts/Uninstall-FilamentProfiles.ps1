...
.PARAMETER ScanOnly
    Scan BambuStudio for custom profiles matching patterns from this repository without removing them.
...
.NOTES
    ...
    - Name pattern (e.g., "@BBL H2D" for H2D profiles)
    - setting_id pattern (e.g., ending with "_H2D1", "_H2D2", etc.)
    - Matching entries in the repository's bbl_json_entries.json files
#>
...
$ScriptDir = Join-Path $RepoRoot "profiles"
...
Write-Host "Installed Custom Profiles:" -ForegroundColor Cyan
foreach ($FileItem in $InstalledFiles) {
    Write-Host "  [✓] $($FileItem.Entry.name)" -ForegroundColor Gray
}
Write-Host ""
...
if ($ScanOnly) {
    ...
    Write-Host "  Profiles found:  $($InstalledFiles.Count)" -ForegroundColor White
    Write-Host "  Location:        $DestFilamentDir" -ForegroundColor White
    Write-Host ""
    Write-Host "Run without -ScanOnly to remove these profiles." -ForegroundColor Yellow
    Write-Host ""
    exit 0
}
